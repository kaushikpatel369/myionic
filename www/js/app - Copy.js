...
(function(){
	...
	app.controller('AppCtrl', function($scope, $ionicPopup, $http,$state,$stateParams){
		...
		$scope.delete = function(id) {
			...
			if(res) {
				...
				$http.delete(' http://localhost:8000/contactlist/'+id).then(function(response){
	...
	app.controller('ShowPop', function($scope, $ionicPopup, $http,$state,$stateParams){
		...
		var myPopup = $ionicPopup.show({
				template: '<label class="item item-input item-stacked-label"><span class="inputlabel">Name</span>'
							+'<input type="text" ng-model="contact.name"></label><label class="item item-input itemstacked-label">'+
						  '<span class="input-label">Email</span><input type="text" ngmodel="contact.email"></label>'+
		...
		if(res){
				$http.post(' http://localhost:8000/contactlist',$scope.contact).then(function(response){
					$scope.contact = "";
					$state.go('list');
					//A Alert Popup
					var alertPopup = $ionicPopup.alert({
	...
	app.controller('AddCtrl', function($scope, $stateParams, $http, $state){
		$scope.adduser = function(contact){
			console.log(contact);
			$http.post(' http://localhost:8000/contactlist',contact).then(function(response){
				$scope.contact = "";
				$state.go('list');
			});
		};
	...
	app.controller('EditCtrl', function($scope, $stateParams, $http, $state,$ionicPopup){
			...
			console.log(id);
			$http.get('http://localhost:8000/contactlist/'+id).success(function(response){
				$scope.contact = response;
			});
			$scope.update = function(){
				console.log($scope.contact._id);
				$http.put(' http://localhost:8000/contactlist/' + $scope.contact._id,$scope.contact).success(function(response){
					$state.go('list');
					//A Alert Popup
					var alertPopup = $ionicPopup.alert({
